...
- **Sanity Check**: Connect the same `videoKey` source that feeds CV, DL, or pose services so you can compare visual input against downstream results.
...
- **Source Selection**: If the wrong source branch is connected, you can waste time debugging overlays when the real issue is an upstream video selection mismatch (`videoKey` typo).
