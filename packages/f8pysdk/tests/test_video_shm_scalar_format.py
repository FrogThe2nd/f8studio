...
def test_video_shm_scalar1f32_roundtrip() -> None:
    shm_name = f"test.shm.scalar.{uuid.uuid4().hex}"
    ...
    try:
        ...
        writer.write_frame(width=width, height=height, pitch=pitch, payload=payload, fmt=VIDEO_FORMAT_SCALAR1_F32)

        header, frame = reader.read_latest_frame()
        assert header is not None
        assert frame is not None
        assert int(header.fmt) == VIDEO_FORMAT_SCALAR1_F32
