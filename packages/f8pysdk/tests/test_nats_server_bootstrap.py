...
from f8pysdk.service_runtime_tools.deploy.nats_bootstrap import (  # noqa: E402
    _is_local_host,
...
class NatsServerBootstrapTests(unittest.TestCase):
    ...
    def test_nats_bootstrap_skip_remote_host(self) -> None:
        with patch("f8pysdk.service_runtime_tools.deploy.nats_bootstrap._is_tcp_reachable", return_value=False), patch(
            "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._resolve_nats_server_binary"
        ) as resolve_mock:
    ...
    def test_nats_bootstrap_uses_path_binary_first(self) -> None:
        ...
        with patch("f8pysdk.service_runtime_tools.deploy.nats_bootstrap.shutil.which", return_value=fake):
    ...
    def test_nats_bootstrap_download_when_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("f8pysdk.service_runtime_tools.deploy.nats_bootstrap.shutil.which", return_value=None), patch(
                "f8pysdk.service_runtime_tools.deploy.nats_bootstrap.Path.home", return_value=Path(td)
            ), patch(
                "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._download_latest_release_archive", return_value=Path(td) / "pkg.tar.gz"
            ) as dl_mock, patch(
                "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._extract_archive"
            ) as extract_mock, patch(
                "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._locate_binary", return_value=Path(td) / "extract" / "nats-server"
            ) as locate_mock, patch(
                "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._install_downloaded_binary", return_value=expected
            ) as install_mock:
    ...
    def test_nats_bootstrap_returns_false_on_download_failure(self) -> None:
        with patch("f8pysdk.service_runtime_tools.deploy.nats_bootstrap._is_tcp_reachable", return_value=False), patch(
            "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._resolve_nats_server_binary", side_effect=RuntimeError("boom")
        ):
    ...
    def test_nats_bootstrap_with_result_marks_owned_spawn(self) -> None:
        with patch(
            "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._is_tcp_reachable",
            side_effect=[False, True],
        ), patch("f8pysdk.service_runtime_tools.deploy.nats_bootstrap._resolve_nats_server_binary"), patch(
            "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._spawn_nats_server",
            return_value=43210,
        ):
    ...
    def test_nats_bootstrap_with_result_existing_server_not_owned(self) -> None:
        with patch("f8pysdk.service_runtime_tools.deploy.nats_bootstrap._is_tcp_reachable", return_value=True), patch(
            "f8pysdk.service_runtime_tools.deploy.nats_bootstrap._spawn_nats_server"
        ) as spawn_mock:
            out = ensure_nats_server_with_result("nats://127.0.0.1:4222")
        self.assertTrue(out.reachable)
    ...
    def test_stop_nats_server_process_when_already_stopped(self) -> None:
        with patch("f8pysdk.service_runtime_tools.deploy.nats_bootstrap._is_pid_running", return_value=False):
