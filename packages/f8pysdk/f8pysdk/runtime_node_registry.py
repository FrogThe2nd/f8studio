...
class RuntimeNodeRegistry:
    ...
    def _service_known(self, service_class: str) -> bool:
        ...
        return (
            service_class in self._by_service_operator
            or service_class in self._by_service_service
            or service_class in self._service_specs
            or service_class in self._operator_specs
        )
    ...
    def create_service_node(
        ...
        *,
        ...
        if factory is None:
            if not self._service_known(service_class):
                raise ServiceNotRegistered(service_class)
    ...
    def load_modules(self, modules: list[str]) -> None:
        ...
        - `RuntimeNodeRegistry.instance().register_operator_factory(...)`
        """
        for m in modules:
            name = str(m or "").strip()
            if not name:
