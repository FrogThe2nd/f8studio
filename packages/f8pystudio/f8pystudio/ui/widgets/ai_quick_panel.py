"""Compact AI settings panel for the shared Studio agent sidebar."""
...
from ...agents.model_catalog import supports_agent_chat_model
...
class AiQuickPanel(QtWidgets.QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        layout.addWidget(self._section_label("Agent"))
        ...
        self._chat_refresh_btn.setToolTip("Refresh model catalog")
    ...
    def _fill_chat_models(self, cfg: ProviderConfig) -> None:
        ...
        try:
            ...
            for m in cfg.cached_models:
                if m.health_status == "error" or not supports_agent_chat_model(m):
                    continue
    ...
    def _on_model_tested(self, pid: str, _mid: str, _success: bool, _error: str) -> None:
        ...
        self._building = True
        try:
            idx = self._chat_provider_combo.findData(pid)
            if idx >= 0:
                self._chat_provider_combo.setItemText(idx, f"{cfg.health_icon} {cfg.display_name}")
        finally:
            self._building = False
