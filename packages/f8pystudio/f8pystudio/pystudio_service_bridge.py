...
class PyStudioServiceBridge(QtCore.QObject):
    ...
    def _collect_known_service_ids(self) -> list[str]:
        ...
        known_service_ids.update(self._managed_service_ids)
        known_service_ids.update(self._managed_service_classes.keys())
        known_service_ids.update(self._service_alive_cache.keys())
        known_service_ids.update(self._service_status_cache.keys())
        known_service_ids.update(self._monitor_center.latest_snapshots().keys())
        try:
            known_service_ids.update(self._process_gateway.service_ids())
        except Exception as exc:
    ...
    def list_service_monitor_rows(self) -> list[ServiceMonitorRow]:
        ...
        for service_id in self._collect_known_service_ids():
            latest_snapshot = latest_by_service.get(service_id)
            running = bool(self.is_service_running(service_id))
            alive: bool | None = None
            ready: bool | None = None
            active: bool | None = self.get_cached_service_active(service_id)
            service_class = str(self._managed_service_classes.get(service_id, "") or "").strip()
            ...
            if latest_snapshot is not None:
                ...
                active = bool(latest_snapshot.active) if active is None else active
                ...
                alive_cache = self._service_alive_cache.get(service_id)
