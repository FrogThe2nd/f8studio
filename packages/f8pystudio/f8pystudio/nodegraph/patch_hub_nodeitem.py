...
class F8StudioPatchHubNodeItem(F8StudioOperatorNodeItem):
    ...
    _TOP_PADDING = 4.0
    _BOTTOM_PADDING = 4.0
    _TITLE_PADDING_X = 12.0
    _TITLE_TO_BODY_GAP = 14.0
    _PORT_LANE_WIDTH = 27.0
    _LABEL_PADDING_X = 4.0
    ...
    def _set_base_size(self, add_w=0.0, add_h=0.0):  # type: ignore[override]
        old_rect = None
        try:
    ...
    def _calc_size_horizontal(self):  # type: ignore[override]
        ...
        for port, text in self._input_items.items():
            try:
                ...
                if port_name.startswith("[D]") or port_name.startswith("[S]"):
                    ...
                    label_h = max(label_h, float(text.boundingRect().height()))
            except (AttributeError, RuntimeError, TypeError, ValueError):
                continue
        ...
        row_height = max(float(PortEnum.SIZE.value), float(port_height or 0.0), float(label_h or 0.0))
        row_step = self._ROW_STEP
        group_gap = self._GROUP_GAP
        ...
        body_height = 0.0
        if total_rows > 0:
            body_height = self._TOP_PADDING + row_height + (float(max(0, total_rows - 1)) * row_step)
        if group_count > 1:
            body_height += float(group_count - 1) * group_gap
        if body_height > 0.0:
            body_height += self._BOTTOM_PADDING

        width = max(
            self._MIN_WIDTH,
            float(text_w + self._TITLE_PADDING_X),
            float(port_width + self._PORT_LANE_WIDTH),
            float(label_w + self._LABEL_PADDING_X),
        )
    ...
    def _align_ports_horizontal(self, v_offset):  # type: ignore[override]
        ...
        y = float(v_offset) + self._TOP_PADDING
        groups = (
        ...
        if wrote_group:
            self._ports_end_y = y - row_step + row_height
        else:
            self._ports_end_y = y
