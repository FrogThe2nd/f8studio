...
def register_operator(registry: RuntimeNodeRegistry) -> RuntimeNodeRegistry:
    ...
    def _factory(node_id: str, node: F8RuntimeNode, initial_state: dict[str, Any]) -> OperatorNode:
        return HighpassFilterRuntimeNode(node_id=node_id, node=node, initial_state=initial_state)

    registry.register_operator_factory(SERVICE_CLASS, OPERATOR_CLASS, _factory, overwrite=True)
    registry.register_operator_spec(HighpassFilterRuntimeNode.SPEC, overwrite=True)
    return registry
